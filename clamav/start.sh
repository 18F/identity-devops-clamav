...
watchnondocker() {
	inotifywait -e close_write -e create --format %w%f -r -m --exclude '\/host-fs\/var\/lib\/docker\/|\/host-fs\/proc\/|\/host-fs\/sys\/|\/host-fs\/dev\/|\/host-fs\/run\/' /host-fs | grep -vf /ignore.txt | while read line ; do
...
watchdocker() {
	inotifywait -e close_write -e create --format %w%f -r -m "$1" | grep -vf /ignore.txt | while read line ; do
		clamdscan --no-summary --fdpass --stdout --infected "$line" 2>&1 | grep -v ': OK'
	done
}
while true ; do
